...
\def \ProveConvRecur {% П-ть задана рекурентно, д-ть сходимость
        ...
        $\begin{cases}
            ...
            x_1 = 1     
        \end{cases}$\\
        (\textit{Указание: Последовательность $y_n = \sum \limits_{k = 0}^n \dfrac{1}{k!}$} сходится.)
        }
...
\def \FindAllSubLimits {% Найти все чп п-тей
        ...
        $2)\; y_n = n^{\sin \frac{\pi n}{2}}$
        }
\def \FindExampleContableSubsequence {% Привести пример п-ти, у кот. мн-во ч.п. счетно
        \item Привести пример последовательности, у которой множество всех ее частичных пределов счетно.
        }
\def \FindExamleSubsequence {% Привести пример п-ти, у которой м-во ч.п. 0, +-1,2
        \item Привести пример последовательности со следующим множеством частичных пределов $A = (0, \pm 1, \pm 2 )$.
        }
...
\def \ProveFundSequence {% Док-ть фунд п-ти
        \item Исследовать последовательность $x_n = n \cos \pi n$ на ограниченность, сходимость и фундаментальность по определению.
        }
